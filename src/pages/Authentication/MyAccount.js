...
const MyAccount = props => {
  ...
  const states = useSelector(state => state.countrysData.states)
    .filter(item => item.countryId == validation.values.countryId)
    .map(item => {
      return { label: item.name, value: item.id, countryId: item.countryId }
    })

  const citys = useSelector(state => state.countrysData.citys)
  ...
  const onProfileSelect = e => {
    ...
    if (format.includes(true)) {
      ...
      return alert(
        "Image not size is minimum 5mb or image format is not (jpg,jpeg,png)."
      )
    }
  }
  ...
  React.useEffect(
    () => {
      citys.filter(ele => ele.stateId == validation.values.stateId)
    },
